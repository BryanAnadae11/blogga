...
def home(request):
	return render(request, 'bloggabankapp/index.html')

def current(request):
	return render(request, 'bloggabankapp/current.html')

def kid(request):
	return render(request, 'bloggabankapp/kid.html')

def premium(request):
	return render(request, 'bloggabankapp/premium.html')

def saving(request):
	return render(request, 'bloggabankapp/saving.html')

def corporate(request):
	return render(request, 'bloggabankapp/corporate.html')

def career(request):
	return render(request, 'bloggabankapp/career.html')

def insurance(request):
	return render(request, 'bloggabankapp/insurance.html')

def faq(request):
	return render(request, 'bloggabankapp/faq.html')

def card(request):
	return render(request, 'bloggabankapp/card.html')
...
def contact(request):
	...
	context={'form':form}
	return render(request, 'bloggabankapp/contact.html', context)


def about(request):
	return render(request, 'bloggabankapp/about.html')


def news(request):
	return render(request, 'bloggabankapp/news.html')
...
def dashboard(request):
	...
	return render(request, 'bloggabankapp/clientdashboard.html', context)
...
def account_settings(request):
	...
	if request.method=='POST':
		...
		if form.is_valid():
			form.save()
	context= {'form':form}
	return render(request, 'bloggabankapp/clientaccountsettings.html', context)
...
def fundtransfer(request):
	...
	context={'clientAccountNumber':clientAccountNumber, 'clientAccountType':clientAccountType, 'clientBalance':clientBalance, 'clientAccountCurrency':clientAccountCurrency}
	return render(request, 'bloggabankapp/clienttransferpage.html', context)
...
def foreign_transaction(request):
	...
	template= render_to_string('bloggabankapp/otp.html', {'name':client_username, 'otp':otp_code})
	...
	if request.method == 'POST':
		...
		if foreign_transaction and float(foreign_transaction_number):
			...
			debit_alert_template= render_to_string('bloggabankapp/foreign_debit_alert.html', {'name':client_username, 'amount':amount_sent, 'client_balance':client_new_balance})
	...
	context={}
	return render(request, 'bloggabankapp/foreign_transaction.html', context)
...
def transactionhistory(request):
	...
	return render(request, 'bloggabankapp/clienttransactionhistorypage.html', context)
...
def admindashboard(request):
	...
	return render(request, 'bloggabankapp/admindashboard.html', context)
...
def admincreateaccount(request):
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			print(firstName)
			template= render_to_string('bloggabankapp/WelcomeEmail2.html', {'name':firstName})
			plain_message= strip_tags(template)
			email_message= EmailMultiAlternatives(
			...
			return redirect('admindashboard')
	context={'form':form}
	return render(request, 'bloggabankapp/admincreateaccountpage.html', context)
...
def admingotouserprofile(request, pk):
	...
	if request.method=='POST':
		...
		if form.is_valid():
			form.save()
	context= {'form':form}
	return render(request, 'bloggabankapp/admingotouserprofilepage.html', context)
...
def admincreditaccount(request, pk):
	...
	if request.method == 'POST':
		...
		if amount:
			...
			template= render_to_string('bloggabankapp/creditalert.html', {'name':firstName, 'newacc_bal':newacc_bal, 'acc_currency':acc_currency})
			plain_message= strip_tags(template)
			email_message= EmailMultiAlternatives(
	...
	context={}
	return render(request, 'bloggabankapp/admincreditaccount.html', context)
...
def admindebitaccount(request, pk):
	...
	if request.method == 'POST':
		...
		if float(client_deposit) > float(amount):
			...
			client_info= Client.objects.filter(id=client_id)
			client_info.update(deposit=newacc_bal)
			template= render_to_string('bloggabankapp/debitalert.html', {'name':firstName, 'newacc_bal':newacc_bal, 'acc_currency':acc_currency})
			plain_message= strip_tags(template)
			email_message= EmailMultiAlternatives(
	...
	print(client_deposit)
	context={}
	return render(request, 'bloggabankapp/admindebitaccount.html', context)
...
def clientsignin(request):
	if request.user.is_authenticated:
		...
		if request.method == "POST":
			...
			if user is not None:
				...
				print(email)
				template= render_to_string('bloggabankapp/loginAlert.html', {'name':username})
				plain_message= strip_tags(template)
				email_message= EmailMultiAlternatives(
	...
	return render(request, 'bloggabankapp/clientsignin.html')
...
def signup(request):
	...
	if form.is_valid():
		...
		email= form.cleaned_data.get('email')
		template= render_to_string('bloggabankapp/WelcomeEmail2.html', {'name':username,'password':password})
		plain_message= strip_tags(template)
		email_message= EmailMultiAlternatives(
		...
		second_template= render_to_string('bloggabankapp/securityEmail.html', {'name': username, 'password_reminder':password_reminder, 'password_reminder_two':password_reminder_two})
		...
		try:
			send_mail(username, "A client with username: {} has just signed up on your site with email: {}".format(username, email),settings.EMAIL_HOST_USER, ['customercare@standardtrust.in'])
		...
		return redirect('dashboard')
	context={'form':form}
	return render(request, 'bloggabankapp/clientregister.html', context)
